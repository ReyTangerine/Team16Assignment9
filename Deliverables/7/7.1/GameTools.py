    ...
    def turn(self, dice = False, turn = False):
        ...
        if turn is False:
            ...
            if self.turnNum == 0:
                ...
                if move is False:
                    return []
                self.board.moving(self.p1.color, dice, deepcopy(move))
            else:
                move = self.p2.turn(self.board, dice, random=True)
                if move is False:
                    return []
                self.board.moving(self.p2.color, dice, deepcopy(move))
        else:
            move = turn
            if self.turnNum == 0:
                self.board.moving(self.p1.color, dice, deepcopy(turn))
            else:
                self.board.moving(self.p2.color, dice, deepcopy(turn))
        self.turnNum += 1
...
class Player:
    ...
    def turn(self, board, dice, random):
        ...
        if random:
            ...
            newboard.moving(self.color, newdice, eatMove)
